<template>
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm dark:shadow-none transition-shadow hover:shadow-xl">
    <!-- Edit mode -->
    <div v-if="editing" class="space-y-3">
      ...
      <div class="flex justify-end gap-2">
        <button @click="editing = false" class="text-sm text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 px-3 py-1.5 transition-colors">
          {{ t('card.cancel') }}
        </button>
        <button
          @click="save"
          class="text-sm bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900 px-4 py-1.5 rounded-lg hover:bg-gray-700 dark:hover:bg-white transition-colors"
        >
          {{ t('card.save') }}
        </button>
      </div>
    ...
    <template v-else>
      <div class="flex items-start justify-between gap-3">
        <img
          ...
          :src="entry.image_url"
          alt=""
          class="w-14 h-20 object-cover rounded-md border border-gray-200 dark:border-gray-700 shrink-0"
        />
        <div class="flex-1 min-w-0">
          <p class="font-medium text-gray-900 dark:text-gray-100 truncate">{{ entry.title }}</p>
          ...
          <p v-if="entry.notes" class="text-sm text-gray-500 dark:text-gray-400 mt-2 leading-relaxed">{{ entry.notes }}</p>

          <div v-if="otherScores.length" class="mt-2 space-y-0.5">
            <p v-for="s in otherScores" :key="s.id" class="text-xs text-gray-400 dark:text-gray-500">
              <span class="font-medium text-gray-500 dark:text-gray-400">{{ s.user?.username }}</span>
        ...
        <div class="flex gap-3 shrink-0 pt-0.5">
          <button @click="startEdit" class="text-xs text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 transition-colors">{{ t('card.edit') }}</button>
      ...
      <div class="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
        <button
          ...
          class="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 flex items-center gap-1.5 transition-colors"
        >
          <span>{{ expanded ? '▲' : '▼' }}</span>
          <span>{{ t('card.history') }} · {{ entry.history.length }} {{ entry.history.length === 1 ? t('card.event') : t('card.events') }}</span>
        </button>
        <ul v-if="expanded" class="mt-2.5 space-y-1.5 pl-3 border-l-2 border-gray-100 dark:border-gray-700">
